...
\begin{document}
	...
	\textbf{Concept:} Because standard Transformers process all $c$ tokens in parallel, we must inject sequence information so the model knows the order of words.
	...
	This block constitutes the core routing mechanism of the Transformer, allowing tokens to share information across the context window $c$.
	...
	\textbf{Concept:} SSMs track a compressed hidden state $h_t$ over time.
	...
	\textbf{Concept:} Computes $Q (K^T V)$ instead of $(Q K^T) V$ to avoid $c \times c$ scaling, using a running memory $S_t$.
	...
	\textbf{Concept:} Adds the input of a layer directly to its output to prevent gradients from vanishing in deep networks.
	...
	Forces activations to have stable variance so that numbers do not grow infinitely large across many layers.
	...
	\begin{itemize}
		...
		\item $\text{ReLU}(x)$: Rectified Linear Unit, an activation function defined as $\max(0, x)$.
	...
	Adam adjusts the parameters $\theta$ by tracking moving averages of the gradients.
	\begin{equation}
	...
	An alternative optimizer specifically designed for internal weight matrices ($W$).
	\begin{equation}
